...
using static Content.Shared.Genetics.Components.SharedGeneticsConsoleComponent;
...
namespace Content.Client.GeneticsConsole.UI
{
    ...
    public sealed partial class GeneticsConsoleWindow : FancyWindow
    {
        ...
        private static readonly Dictionary<Base, Color> BaseToColor = new Dictionary<Base, Color>
            {
                { Base.A, Color.FromHex("#DAE8FC") },
                { Base.T, Color.FromHex("#FFCBB5") },
                { Base.G, Color.FromHex("#F8B3DF") },
                { Base.C, Color.FromHex("#CDEB8B") },
                { Base.Unknown, Color.FromHex("#F55555") },
        ...
        public void SetTargetGene(Gene gene, Dictionary<long, string> knownMutations, GenePuzzle? puzzle)
        {
            ...
            var geneString = string.Join(" ", gene.Blocks.Select(b => DecimalToGene(b.Value)));
        ...
        public void SetGeneRepairPanel(List<Gene> genes, Dictionary<long, string> knownMutations)
        {
            ...
            foreach (var gene in genes)
            {
                ...
                if (gene.Blocks.Count > 0)
                {
                    ...
                    if (gene.Type != GeneType.Mutation)
                    {
                        if (gene.Damaged)
                        {
